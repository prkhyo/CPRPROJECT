    document.addEventListener("DOMContentLoaded", function(e){
        ...
        window.onclick=function(){
            for(var e=document.getElementsByClassName("selectbox-options"), t=0;t<e.length;t++)
                e[t].setAttribute("class","selectbox-options selectbox-options--hidden")
        };
        for(var s=document.getElementsByClassName("justselect1"),n=0;n<s.length;n++){
            s[n].required=!0, s[n].onchange=function(){
                var e=document.getElementsByClassName("selectbox__label"), t=this.value,s=this.dataset.sid,n=this.options;
                for(a=0;a<e.length;a++)
                    if(e[a].parentElement.dataset.pair===s){
                        for(b=0;b<n.length;b++)
                            if(n[b].value==t){e[a].innerHTML=n[b].innerHTML;
                                break
                            }
                            break
                        }
            ...
            !function(e,t){
                e.parentNode.insertBefore(t,e.nextSibling)}(s[n],l),s[n].setAttribute("data-sid","select-"+(n+1)),s[n].style.display="none"
        }

        for(var u=document.getElementsByClassName("justselect1"),p=0;p<u.length;p++){
            var m=document.createElement("div");m.setAttribute("class","justwrap");
        ...
        for(var v=document.getElementsByClassName("selectbox__label"),x=0;x<v.length;x++)
